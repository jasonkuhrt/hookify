...
} from "../packages/install/src/index.ts";
...
const main = async (): Promise<void> => {
  const homeDirectory = resolve(process.env.HOME ?? homedir());
  const hookifyHome = resolve(
    process.env.HOOKIFY_HOME ?? join(homeDirectory, ".local", "share", "hookify"),
  );
  const repoPath = resolve(process.env.HOOKIFY_REPO_DIR ?? join(hookifyHome, "repo"));
  ...
  const provenance = resolveHookifyInstallProvenance({
    ...
    installedVia: process.env.HOOKIFY_INSTALLED_VIA ?? "script:install-codex.sh",
  });
  ...
  const installedPluginId = `hookify@${marketplaceName}`;

  process.stdout.write(
    [
      "Hookify for Codex installed.",
      `Plugin id: ${installedPluginId}`,
      `Generated plugin: ${generatedPluginPath}`,
      `Home plugin link: ${pluginLinkPath}`,
      `Generated dispatcher: ${dispatcherPath}`,
      `Marketplace: ${personalMarketplacePath}`,
      `Codex config: ${codexConfigPath}`,
      `Codex hooks cleanup: ${codexHooksPath}`,
